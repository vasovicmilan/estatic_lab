...
- Stranicu "katalog" koja navodi usluge, pakete, i (kad kod pokriva artikle) proizvode sa sajta, svaki sa već ugrađenim ličnim referalnim linkom spremnim za kopiranje — tako da deljenje konkretne ponude ne zahteva ručno sastavljanje linka. Lista proizvoda je **filtrirana na osnovu stvarne podobnosti** za partnerov kod — poštuje i belu listu (`applicableProducts`, ako je podešena) i isključene kategorije artikala (pogledajte `05-kuponi-i-popusti.md`), tako da partner nikad ne dobije link koji izgleda kao da radi, a na checkout-u tiho ne važi. Ako kod isključuje neku kategoriju, stranica to i eksplicitno navodi po imenu, umesto da artikli iz nje jednostavno nestanu bez objašnjenja.