...
- A "catalog" page listing services, packages, and (when the code covers products) products from the site, each with their personal referral link already built in and ready to copy — so sharing a specific offer doesn't require constructing a link by hand. The product list is **filtered to actual eligibility** for the partner's code — it respects both the whitelist (`applicableProducts`, if set) and any excluded product categories (see `05-coupons-and-discounts.md`), so a partner never gets a link that looks like it works but silently doesn't discount at checkout. When a code excludes a category, the page names it explicitly rather than just having those items silently vanish with no explanation.