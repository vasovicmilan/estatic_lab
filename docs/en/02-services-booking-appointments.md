...
This flow reuses exactly the same mechanics as public booking (the transactional availability, resource, and overlap checking, automatic/manual staff assignment) — the only differences are where the request comes from, and two additional options public booking doesn't have:

- **Manual price override** — available to admins and employees only (never to public booking), and deliberately **excludes** coupons and package payment for that appointment - combining them would be both mathematically wrong (a coupon discounts the catalog price by a percentage/fixed amount, it doesn't replace it outright) and a security concern (a coupon is a code that could theoretically be replayed; a manual price never leaves the admin panel). An appointment created this way is flagged (`manualBooking` in the database, shown on the appointment's detail page) for transparency and reporting - including whether the performing staff member's commission on it gets the minimum guaranteed floor, see `07-employee-compensation.md`.
- **Paying from the customer's existing package** — when an existing registered customer is selected along with a service/variant that one of their purchased packages covers, the form automatically detects this (the same check public booking uses, `findUsablePurchaseForService`) and offers paying from that package instead of separately - exactly the same mechanism described above under "What booking costs" (the session is reserved at creation, actually consumed only once the appointment is completed). This is deliberately only offered when a manual price override is **not** used - the two are mutually exclusive, same as a coupon.
...
- The date/time field is always interpreted as the local (Belgrade) wall-clock time the admin actually entered/picked, regardless of which timezone the server process itself runs in - the same rule scheduled blog posts follow (`zonedInputToUtcDate`, see `src/utils/date.time.util.js`).
