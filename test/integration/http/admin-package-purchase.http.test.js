...
async function createServiceWithVariant() {
  return serviceRepo.createService({
    name: "Masaza",
    slug: "masaza",
    image: { img: "/images/services/x.webp", imgDesc: "x" },
    packages: [{ name: "Relax", slug: "relax", duration: 60, totalPrice: 3000, isActive: true }],
    isActive: true,
  });
}
...
describe("admin package CRUD + image upload (HTTP)", () => {
  ...
  it("creates a package with an uploaded image and a JSON-encoded items array", async () => {
    ...
    const service = await createServiceWithVariant();
    ...
    const res = await agent
      ...
      .field(
        "items",
        JSON.stringify([{ service: service._id.toString(), servicePackageId: service.packages[0]._id.toString(), sessions: 1 }])
      )
    ...
    assert.equal(String(result.data[0].items[0].servicePackageId), service.packages[0]._id.toString());
  ...
  it("rejects an item with no servicePackageId", async () => {
    const agent = request.agent(app);
    await registerAndLogin(agent, { email: "admin@example.com", roleName: "admin" });
    const service = await createServiceWithVariant();
    const { token } = await getCsrfToken(agent, "/admin/paketi/dodavanje");
    ...
    const res = await agent
      .post("/admin/paketi")
      .field("CSRFToken", token)
      .field("name", "Bez Varijante")
      .field("description", "Opis")
      .field("totalPrice", "1000")
      .field("items", JSON.stringify([{ service: service._id.toString(), sessions: 1 }]));
  ...
  it("edits an existing package", async () => {
    ...
    const res = await agent
      ...
      .field(
        "items",
        JSON.stringify([{ service: service._id.toString(), servicePackageId: service.packages[0]._id.toString(), sessions: 2 }])
      );
