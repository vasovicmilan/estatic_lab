...
export async function getBlogLandingData({ limit = 9, page = 1, search = "" } = {}) {
  ...
  const seo = buildPageSeo({
    title: "Blog | Estatic Lab",
    description: "Saveti o nezi, wellness rutinama i tretmanima — pratite Estatic Lab blog.",
    canonical: "/blog",
    isIndexable: true,
  });
...
export async function getBlogCategoryData(categorySlug, { limit = 9, page = 1 } = {}) {
  ...
  const seo = buildPageSeo({
    title: `${category.name} | Blog | Estatic Lab`,
...
export async function getBlogTagData(tagSlug, { limit = 9, page = 1 } = {}) {
  ...
  const seo = buildPageSeo({
    title: `#${tag.name} | Blog | Estatic Lab`,
...
export async function getBlogPostData(slug) {
  ...
  const seo = buildPageSeo({
    title: post.seo?.naslov || `${post.naslov} | Blog | Estatic Lab`,
...
export async function searchBlogPosts(search, { limit = 9, page = 1 } = {}) {
  ...
  const seo = buildPageSeo({
    title: `Pretraga: ${search} | Blog | Estatic Lab`,
    description: `Rezultati pretrage za "${search}" na Estatic Lab blogu.`,
